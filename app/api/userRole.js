...
const { validateBody } = require('../validation/userRole')
...
module.exports = app => {
  ...
  api.create = async (req, res) => {
    ...
    if (errors) {
      return res.status(400).json(error.parse('userRole-400', generateValidationErrorMessage(errors)))
    }

    //try to create
    try {
      const created = await db.UserRole.create(req.body)
  ...
  api.read = async (req, res) => {
    const toRead = await db.UserRole.findByPk(req.params.id)

    //verify valid id
    if (!toRead) {
      return res.status(400).json(error.parse('userRole-400', idNotFoundErrorMessage()))
    }
  ...
  api.update = async (req, res) => {
    const toUpdate = await db.UserRole.findByPk(req.params.id)
  ...
  api.delete = async (req, res) => {
    const toDelete = await db.UserRole.findByPk(req.params.id)
