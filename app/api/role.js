...
const { validateBody } = require('../validation/role')
...
module.exports = app => {
  ...
  api.create = async (req, res) => {
    ...
    if (errors) {
      return res.status(400).json(error.parse('role-400', generateValidationErrorMessage(errors)))
    }
  ...
  api.read = async (req, res) => {
    ...
    //verify valid id
    if (!toRead) {
      return res.status(400).json(error.parse('role-400', idNotFoundErrorMessage()))
    }
